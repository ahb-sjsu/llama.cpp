...
- [ ] **Sprint 4c** — Wire `tiered_cache` into `llama_kv_cache` ← *in progress* (plan: [`turboquant-kv-sprint4c-plan.md`](turboquant-kv-sprint4c-plan.md))
  ...
  - [ ] Step 3c-5c — shrink/dynamically-size view tensor (to `n_kv` per forward pass, not `kv_size`). This is the step that finally delivers net VRAM savings since backbone is already tiny after 3c-5b
