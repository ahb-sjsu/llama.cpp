...
- [ ] **Sprint 4c** — Wire `tiered_cache` into `llama_kv_cache` ← *in progress* (plan: [`turboquant-kv-sprint4c-plan.md`](turboquant-kv-sprint4c-plan.md))
  ...
  - [⚠️] **Step 3c-5c investigation** — attempted `LLAMA_TQ_SHRINK_VIEW=N` (allocate view tensor smaller than kv_size) and uncovered a deeper architectural issue: **the previous view-bind "0% disagreement" results were a false positive.** The env-var reads happened AFTER the layer-loop allocation, so `tq_view_k_/v_` were never actually allocated — the view-bind code path was dead, and attention still read from the backbone (which had cpy_k's fresh writes). Fixing the env-read order made view-bind real and exposed that **cpy_k writes to the backbone while attention reads from the view — they're disconnected**. The view is only populated by post_compute (one batch behind), so attention reads stale data and output diverges. Fixing requires making cpy_k write directly to the view (and observation also read from the view, keeping backbone + view consistent) — tried this in-turn but ran out of scope before getting it right. Deferred to a focused 3c-5c sprint. Reverted to 3c-5b state which is known-good
  - [ ] Step 3c-5c (redo) — cpy_k writes to view tensor directly; observation reads from view; backbone becomes truly unused and can be eliminated. Gating evidence: logit match with fp16 at hot_window = view_size (all-hot, lossless)
